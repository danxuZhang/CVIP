...
int main() {
  ...
  int border = ksize / 2;
  Mat paddedDemoImage =
      Mat::zeros(Size(height + border * 2, width + border * 2), CV_8UC1);
  copyMakeBorder(paddedDemoImage, paddedDemoImage, border, border, border,
  ...
  Mat dilatedImage = paddedDemoImage(Range(border, border + height),
